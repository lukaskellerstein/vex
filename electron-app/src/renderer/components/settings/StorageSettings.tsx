...
import { Image, Trash2, AlertTriangle } from 'lucide-react'
          ...
          <div key={seg.label} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <div
              style={{
                ...
                border: seg.color === 'var(--surface-elevated)' ? '1px solid var(--border)' : 'none',
                flexShrink: 0,
              }}
            />
            <span style={{ fontSize: '12px', color: 'var(--foreground-muted)' }}>{seg.displayLabel}</span>
          </div>
        ))}
      </div>
    </div>
...
function ConfirmDialog({
  ...
  return (
    ...
    >
      ...
      >
        <AlertTriangle size={24} style={{ color: 'var(--status-error)', marginBottom: '12px' }} strokeWidth={1.5} />
        <div
          style={{
            fontSize: '18px',
            fontWeight: 700,
...
export function StorageSettings() {
  ...
  useEffect(() => {
    window.electronAPI.getStorageStats().then((stats: Record<string, unknown>) => {
      if (!stats) return
      ...
      setSegments([
        { label: 'database', value: dbSize, color: 'var(--primary)', displayLabel: `Database: ${formatBytes(dbSize)}` },
        { label: 'screenshots', value: ssSize, color: 'var(--status-info)', displayLabel: `Screenshots: ${formatBytes(ssSize)}` },
        { label: 'free', value: freeEstimate, color: 'var(--surface-elevated)', displayLabel: `Free: ${formatBytes(freeEstimate)}` },
      ])
    }).catch(() => {
      setSegments([
        { label: 'database', value: 24 * 1024 * 1024, color: 'var(--primary)', displayLabel: 'Database: 24 MB' },
        { label: 'screenshots', value: 142 * 1024 * 1024, color: 'var(--status-info)', displayLabel: 'Screenshots: 142 MB' },
        { label: 'free', value: 834 * 1024 * 1024, color: 'var(--surface-elevated)', displayLabel: 'Free: 834 MB' },
      ])
    })
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '24px', marginBottom: '16px' }}>
        <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--foreground)', marginBottom: '16px' }}>
          Storage Usage
        </div>
      ...
      >
        <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--status-error)', marginBottom: '12px' }}>
          Danger Zone
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.background = 'color-mix(in srgb, var(--status-warning) 10%, transparent)'
              e.currentTarget.style.borderColor = 'color-mix(in srgb, var(--status-warning) 60%, transparent)'
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.background = 'transparent'
              e.currentTarget.style.borderColor = 'color-mix(in srgb, var(--status-warning) 30%, transparent)'
            }}
          >
          ...
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.background = 'color-mix(in srgb, var(--status-error) 10%, transparent)'
              e.currentTarget.style.borderColor = 'color-mix(in srgb, var(--status-error) 60%, transparent)'
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.background = 'transparent'
              e.currentTarget.style.borderColor = 'color-mix(in srgb, var(--status-error) 30%, transparent)'
            }}
          >
