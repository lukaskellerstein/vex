...
1) the agent-orchestrator should already use/trigger claude agent sdk  agents - /home/lukas/Projects/Github/lukaskellerstein/my-claude-code/claude_agent_sdk/python
...
1. The VEX extension dialog should have not have the list of actions recorded. That should be moved into the VEX flowtable popup on the page, where we have now only the "modes". We should add the expandable chevron, when clicked it should open bigger panel with all the actions recorded.

2. When any mode is activated, no links on the page should work, if often happens for me that I am getting redirected while just clicking around.
