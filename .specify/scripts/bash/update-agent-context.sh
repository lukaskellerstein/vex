...
# shellcheck disable=SC1091  # sourced path is dynamic; vars are eval-assigned, so following it only adds false SC2153
...
get_project_structure() {
  ...
  if [[ "$project_type" == *"web"* ]]; then
    # shellcheck disable=SC2028  # literal \n is intended: the value is substituted into a sed replacement, which turns it into a newline
    echo "backend/\\nfrontend/\\ntests/"
  else
    # shellcheck disable=SC2028  # literal \n is intended: the value is substituted into a sed replacement, which turns it into a newline
    echo "src/\\ntests/"
...
create_new_agent_file() {
  ...
  local escaped_lang
  # shellcheck disable=SC2016  # single quotes are required: $ and & must stay literal
...
update_existing_agent_file() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$line" =~ \*\*Last\ updated\*\*:.*[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] ]]; then
      # shellcheck disable=SC2001  # character classes: not expressible as ${var//…}
